...
export default function Gallery() {
  ...
  const adminGallery = (settings?.gallery || [])
    .filter((item) => item?.url && item.url.trim() !== "")
    .map((item, index) => ({
      ...item,
      alt: item.alt?.trim() ? item.alt : `Wedding photo ${index + 1}`,
      className: index === 0 ? "col-span-2 row-span-2" : index === 3 ? "col-span-1 row-span-2" : "col-span-1 row-span-1"
    }));

  const photos = adminGallery.length > 0 ? adminGallery : defaultPhotos;
    ...
    <section className="py-24 px-6 font-serif bg-jawa-ivory jawa-pattern">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16 space-y-4">
          ...
          >
            <ImageIcon className="w-6 h-6 text-jawa-maroon" />
          </motion.div>
          <h2 className="text-4xl md:text-5xl font-bold text-jawa-dark tracking-tight">Galeri Foto</h2>
          <p className="text-jawa-brown italic text-lg">Momen-momen indah yang kami abadikan...</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 auto-rows-[200px] md:auto-rows-[250px]">
          {photos.map((photo, index) => (
            <motion.div
              ...
              className={`relative group overflow-hidden rounded-3xl shadow-lg border border-jawa-gold/35 jawa-frame bg-jawa-cream/60 ${photo.className}`}
            >
              <div className="absolute inset-0 bg-[#3b2a1f]/20 group-hover:bg-transparent transition-colors duration-500 z-10" />
              <img
                src={photo.url}
                alt={photo.alt}
                className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-1000 ease-out"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#3b2a1f]/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20 flex items-end p-6">
